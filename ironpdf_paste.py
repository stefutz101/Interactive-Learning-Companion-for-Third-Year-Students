...
for pdf_file in os.listdir(pdf_directory):
    if pdf_file.endswith(".pdf"):
        ...
        # Copy to clipboard the extracted text
        pyperclip.copy(all_text)

        # Create a corresponding .txt file
        txt_file_path = os.path.splitext(pdf_path)[0] + ".txt"
        with open(txt_file_path, "w", encoding="utf-8") as txt_file:
            pass  # Create an empty file for editing

        # Open the .txt file in an editor and wait for text to be pasted
        print(f"Please paste the extracted text into '{txt_file_path}' and save the file.")
